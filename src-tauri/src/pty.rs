...
impl PtyRegistry {
    ...
    pub fn spawn(
        ...
        // not PE images and cannot be launched via ConPTY's CreateProcessW
        // directly; they must run through `cmd.exe /c`. Native `.exe`s spawn
        // as-is. (which_path resolves agents to their `.cmd` shim on Windows.)
        #[cfg(windows)]
        let mut cmd = {
